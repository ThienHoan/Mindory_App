...
function statusBadge(status: RewardRedemption['status']) {
    if (status === 'requested') {
        return <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-black text-slate-600">Chờ ba mẹ duyệt</span>
    }
    if (status === 'approved') {
        return <span className="rounded-full bg-blue-100 px-2 py-0.5 text-xs font-black text-blue-600">Chờ ba mẹ trao quà</span>
    }
    if (status === 'fulfilled') {
        return <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-black text-emerald-600">Đã nhận quà</span>
    }
...
export default function ChildRewardsPage() {
    ...
    async function redeem(item: RewardItem) {
        if (!child || redeemingItemId) return
        ...
        <div className="mx-auto max-w-6xl space-y-6 p-8">
            <section className="rounded-3xl bg-gradient-to-r from-purple-600 to-pink-500 p-6 text-white shadow-lg shadow-purple-100">
                ...
                <p className="mt-2 text-sm font-bold text-purple-100">
                    Bé đang có <span className="text-white">{xp} XP</span>. Chọn món quà mình thích nhé!
                </p>
            </section>
            ...
            <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                {items.length === 0 ? (
                    <div className="rounded-3xl border border-dashed border-purple-200 bg-white p-8 text-center text-sm font-bold text-slate-400 md:col-span-2 xl:col-span-3">
                        Ba mẹ chưa tạo quà nào.
                    </div>
                ) : items.map((item) => {
                        ...
                        <article key={item.id} className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-black/5">
                            <div className="flex items-start justify-between gap-3">
                                <div>
                                    ...
                                    <p className="mt-1 line-clamp-3 text-sm text-slate-500">{item.description || 'Phần thưởng đặc biệt từ ba mẹ.'}</p>
                                </div>
                                <span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-black text-amber-600">{item.cost_points} XP</span>
                            </div>
                            <button
                                onClick={() => redeem(item)}
                                disabled={!enough || redeemingItemId !== null}
                            ...
                            >
                                {isRedeeming ? 'Đang đổi...' : enough ? 'Đổi quà này' : `Cần thêm ${item.cost_points - xp} XP`}
                            </button>
                        </article>
                    )
                })}
            ...
            <section className="space-y-3">
                ...
                {redemptions.length === 0 ? (
                    <div className="rounded-3xl border border-dashed border-slate-200 bg-white p-6 text-center text-sm font-bold text-slate-400">
                        Chưa đổi quà nào.
                    </div>
